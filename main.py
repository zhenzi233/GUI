...
class GUI(tk.Tk):
    ...
    def start_loop(self):
        ...
        # self.process_gui = ProcessGUI(self.process_index, self.process_total)
    ...
    def select_file(self):
        # self.process_gui.process_label_text.config(text='过程：' + "正在选择文件")
        ...
        if self.file_path and self.check_image_legal():
            ...
            if self.mode == 0:
                # 调用检测方法，传入文件路径
                # self.process_gui.process_label_text.config(text='过程：' + "正在以CLAHESURFG2NN模式处理")
    ...
    def deal_image(self, result):
        ...
        # MessageDialog.process_show('进程','正在进行分析ing')
        #从文件路径获取图片
        img = image.read_image(self.file_path)
        # 从结果获取第一个坐标组
        pos1 = result['pixel1']
        ...
        if not pos1:
            ...
            # C算法
                ...
                if(new_pos1 == []):
                    self.label.config(text='该软件不支持绘制被复制超过三个以上的图像的位置' )
                # self.deal_image_and_draw(point, result['locs'], new_pos2, img)
        # 显示所用的时间
        # tkinter.messagebox.showinfo(title='成功', message='处理该图片用时：'+str(cost_time)+ 's')
    ...
    def deal_image_and_draw(self, point, pos1, pos2, img):
        ...
        regions = self.compare_nparray(img_array, pos1, pos2)
            # 通过获取的坐标组来绘制图片
        # new_image = visualize.draw_regions(img, new_pos[0], new_pos[1])
        checkbox_bool = False
    ...
    #     cv2_img1 = cv2.fastNlMeansDenoisingColored(cv2_img, None, 10, 10, 7, 21)

    #     alpha = 1.5
    #     beta = 0
    #     cv2_img2 = cv2.convertScaleAbs(cv2_img1, alpha=alpha, beta=beta)
    #     return np.asarray(cv2_img2)
        ...
        for index in range(len(reference_points_1)):
            ...
            for range_num_x in range(-range_num, range_num):
                for range_num_y in range(-range_num, range_num):
                    ...
                    if np.array_equal(pos1_color, pos2_color):
                        # print('ok' + ' ' + str(index) + ' ' + str(range_num_x) + ' ' + str(range_num_y))
                        pos1.append([x1 + range_num_x, y1 + range_num_y])
                        pos2.append([x2 + range_num_x, y2 + range_num_y])
                        correct += 1
                    else :
                        # print('no' + ' ' + str(index) + ' ' + str(range_num_x) + ' ' + str(range_num_y))
                        incorrect += 1
                    if not self.process_running:
                        return
        if not self.process_running:
